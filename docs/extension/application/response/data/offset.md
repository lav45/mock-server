# `data` — offset pagination

> Applies when [`response.pagination.type`](../data.md) is `"offset"` (the default).
...
Default page size when the request omits the `per-page` parameter.
...
- `response.pagination.totalPages` - total number of pages.
